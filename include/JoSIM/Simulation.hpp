...
#define NONE_SPECIFIED 4
    ...
    public:
        ...
        std::vector<std::pair<std::string, double>> getJunctionPhases(Matrix &mObj);

        Simulation(Input &iObj, Matrix &mObj, int simN = 0);

        Simulation();

        void make_simulation(Input &iObj, Matrix &mObj, int simN);
