...
std::vector<std::pair<std::string, double>> Simulation::getJunctionPhases(Matrix &mObj){

    std::vector<std::pair<std::string,double>> temp;
    // loop through all junctions:
    for(auto index : mObj.components.junctionIndices) {
        JJ junc = std::get<JJ>(mObj.components.devices.at(index));
        // for each junction determine which terminals are of interest
        // get junction name:
        std::string name = junc.netlistInfo.label_;
        ...
        if(node_count == 2){
            // return the difference between pindex and nindex
            double phaseP = this->x_.at(junc.indexInfo.posIndex_.value());
            double phaseN = this->x_.at(junc.indexInfo.negIndex_.value());
            temp.emplace_back(name,phaseP-phaseN);

        } else if (node_count == 1) {
            //check return either pindex value or negindex value
            if (pIndex) {
                // negative node is grounded
                double phaseP = this->x_.at(junc.indexInfo.posIndex_.value());
                temp.emplace_back(name,phaseP);
            } else {
                // positive node is grounded
                double phaseN = this->x_.at(junc.indexInfo.negIndex_.value());
                temp.emplace_back(name,phaseN);
            }
        }
    }
